...
Core client CRUD is complete, tested, and deployed. The API runs against a persistent SQLite database with full create, read, update, and delete support and meaningful HTTP status codes on every failure path. See [Roadmap](#roadmap) for what's next.
